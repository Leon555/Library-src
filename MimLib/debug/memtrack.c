...
void *DebugAllocMemory(size_t Size, char *FileName, int LineNumber)
{
    ...
    if(ptr != NULL) {
	TrackMemory(MEMTRK_MEMALLOC, 0, ptr, Size, FileName, LineNumber);
...
char *DebugAllocCopyString(char *String, char *FileName, int LineNumber)
{
	...
	TrackMemory(MEMTRK_MEMALLOC, 0, p, Length, FileName, LineNumber);
...
void *DebugReAllocMemory(void *pOldMem,
			 size_t NewSize, char *FileName, int LineNumber)
{
    void *NewPtr = NULL;
    int ItWasntNull = 0;
    char *p;
    ...
    NewPtr = realloc(pOldMem, NewSize + (NewSize > 0 ? sizeof(ALIGN) : 0));

    if(NULL != NewPtr) {
	if(ItWasntNull) {
	    TrackMemory(MEMTRK_MEMFREE,
			KeyStore.lu, NULL, 0, FileName, LineNumber);
	}

	if(NewSize > 0) {
	    TrackMemory(MEMTRK_MEMALLOC,
			0, NewPtr, NewSize, FileName, LineNumber);
...
void DebugReleaseMemory(void *pSource, char *FileName, int LineNumber)
{
    char *p;
    ALIGN KeyStore;
	...
	TrackMemory(MEMTRK_MEMFREE, KeyStore.lu, NULL, 0, FileName, LineNumber);
...
int MemTrkCmp(unsigned long key1, unsigned long key2)
{
    int diff = 0;
...
int MemPrintAllocs(const PAYLOAD * p1, void *p2)
{
    ...
    fprintf(fp,
	    ...
	    p1->Size == 1 ? "e " : "es", p1->LineNumber, p1->FileName);
...
int TrackMemory(MEMTRK_MSG Msg,
		unsigned long Key,
		void *Ptr, int Size, char *FileName, int LineNumber)
{
    int ErrorStatus = 0;
    static FILE *fp = NULL;
	...
	if(NULL == AddMemNode(&MemTree, ThisKey, &EntryBuilder)) {
	    fprintf(fp,
	...
	if(NULL != NodePtr) {
	    ...
	    if(CurrAlloc < 0) {
		fprintf(fp, "ERROR: More memory released " "than allocated!\n");
		fflush(fp);
	    }
	    ...
	    fprintf(fp,
		    ...
		    EntryBuilder.Ptr,
		    EntryBuilder.LineNumber, EntryBuilder.FileName);
	    fflush(fp);
	}
	...
	if(tmt != NULL) {
	    ...
	    strftime(timebuffer,
		     sizeof timebuffer, "%H:%M:%S %Z on %A %d %B %Y", tmt);
	    fprintf(fp, "\n%s\n\n", timebuffer);
	}

	fprintf(fp,
		"Current Allocation: %lu byt%s.\n",
		CurrAlloc, CurrAlloc == 1 ? "e" : "es");

	fprintf(fp,
		"Maximum Allocation: %lu byt%s.\n",
		MaxAlloc, MaxAlloc == 1 ? "e" : "es");

	fprintf(fp, "Nodes currently allocated:\n\n");
...
MEMTREE *AddMemNode(MEMTREE ** Node, unsigned long Key, PAYLOAD * Payload)
{
	...
	*Node = malloc(sizeof **Node);
	...
	if(Diff == 0) {
	    ...
	    Twig = AddMemNode(&Twig->Child[ChildIdx], Key, Payload);
	}
    }
...
int WalkMemTree(MEMTREE * Node,
		int (*Function) (const PAYLOAD *, void *), void *Args)
{
    int error = 0;

    if(Node != NULL) {
	error = WalkMemTree(Node->Child[MEM_LEFTCHILD], Function, Args);
	if(error == 0) {
	    error = (*Function) (&Node->Payload, Args);
	}
	if(error == 0) {
	    error = WalkMemTree(Node->Child[MEM_RIGHTCHILD], Function, Args);
	}
    }
	...
	if((*Node)->Key == Key) {
		...
		if(TopNode->Child[MEM_RIGHTCHILD] == NULL) {
		    ...
		    *Node = TopNode->Child[MEM_LEFTCHILD];
		     ...
		     */
		    Successor = FindSmallest(TopNode->Child[MEM_RIGHTCHILD]);
		    Payload = Successor->Payload;
		    LocalKey = Successor->Key;

		    DeleteMemNode(Node, LocalKey);

		    memcpy(&TopNode->Payload, &Payload, sizeof Payload);
