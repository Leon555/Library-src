...
void toieee(float f, unsigned char buf[4])
{
    double mantf;
    long mantl;
...
float fromieee(unsigned char buf[4])
{
    unsigned long mant;
    int e;
    int s;
...
void toieeed(double d, unsigned char buf[8])
{
    double mantf;
    unsigned long mantl;
    unsigned long mantl2;
    ...
    mantl2hi |=
	((unsigned long)s << (63 - 32)) | ((unsigned long)(e & 0x7ff) <<
					   (52 - 32));
    buf[0] = (unsigned char)(mantl2 & 0xff);
    buf[1] = (unsigned char)((mantl2 >> 8) & 0xff);
    buf[2] = (unsigned char)((mantl2 >> 16) & 0xff);
...
double fromieeed(unsigned char buf[8])
{
    unsigned long mant;
